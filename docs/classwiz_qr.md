...
After the introduction of the QR function, let's look at how QR encodes data. For example, a QR obtained when the fx-JP900CW is in the Mode menu and the cursor is in the **"Basic Calculation"** mode produces the following after scanning the code:
```
...
| T | Tabulation | Spreadsheet data in data table mode, or list data in statistics, distribution, math box, etc. modes
| C | Chart | Specifically for storing matrix/vector data, also used in equation mode
| G | g(x) | Expression of g(x) in function table mode
